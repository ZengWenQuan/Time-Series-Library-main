...
plt.switch_backend('agg') # 使用agg后端，避免在服务器上显示图像
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.lradj == 'type1':
        ...
        if epoch <= warmup_epochs:
            ...
            # 余弦退火阶段
            lr_adjust = {epoch: args.learning_rate /2 * (1 + math.cos((epoch - warmup_epochs) / (args.train_epochs - warmup_epochs) * math.pi))}
    if epoch in lr_adjust.keys():
        lr = lr_adjust[epoch]
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        print('Updating learning rate to {:.8f}'.format(lr))
