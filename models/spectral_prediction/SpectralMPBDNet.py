...
# @register_model('SpectralMPBDNet')
class SpectralMPBDNet(nn.Module):
    """
    Multi-Path Block with Dual branches Network, adapted for spectral prediction.
    Configurable via a YAML file.
    """
    def __init__(self, configs):
        super(SpectralMPBDNet, self).__init__()
        
        # Load model-specific configuration
        if not hasattr(configs, 'model_conf') or not configs.model_conf:
            raise ValueError("SpectralMPBDNet requires a model configuration file specified via --model_conf")
        ...
        print("SpectralMPBDNet Model Initialized successfully.")
