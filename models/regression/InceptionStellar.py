...
@register_model('InceptionStellar')
class InceptionStellar(nn.Module):
    """
    基于Inception1D的恒星光谱数据分析模型
    """
    def __init__(self, configs):
        super(InceptionStellar, self).__init__()
        self.task_name = configs.task_name
        self.seq_len = configs.feature_size
        self.pred_len = configs.label_size
        self.use_pca = configs.use_pca if hasattr(configs, 'use_pca') else False
        
        # 输入通道数
        input_channels = 1  # 默认为1
